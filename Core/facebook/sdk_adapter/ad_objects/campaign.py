...
_special_ad_category_sdk = Campaign.SpecialAdCategories


@cat_enum
class SpecialAdCategories(Enum):
    CREDIT = Cat(_special_ad_category_sdk.credit)
    HOUSING = Cat(_special_ad_category_sdk.housing)
    EMPLOYMENT = Cat(_special_ad_category_sdk.employment)
...
_objective_sdk = Campaign.Objective


@cat_enum
class Objective(Enum):
    APP_INSTALLS = Cat(
        _objective_sdk.app_installs,
        description="Send people to the store where they can download your business's app.",
    )
    BRAND_AWARENESS = Cat(
        _objective_sdk.brand_awareness, description="Increase people's awareness of your business, brand or service.",
    )
    CONVERSIONS = Cat(
        _objective_sdk.conversions,
    ...
    EVENT_RESPONSES = Cat(_objective_sdk.event_responses)
    LEAD_GENERATION = Cat(
        _objective_sdk.lead_generation,
    ...
    LINK_CLICKS = Cat(
        _objective_sdk.link_clicks,
        display_name="Traffic",
        description="Send people from Facebook to any URL you choose, such as your website's landing page, a blog post, app etc.",
    )
    LOCAL_AWARENESS = Cat(_objective_sdk.local_awareness, display_name="Store Traffic")
    MESSAGES = Cat(
        _objective_sdk.messages,
    ...
    OFFER_CLAIMS = Cat(_objective_sdk.offer_claims)
...
_bid_strategy_sdk = Campaign.BidStrategy


@cat_enum
class BidStrategy(Enum):
    COST_CAP = Cat(_bid_strategy_sdk.cost_cap)
    # TODO: 'LOWEST_COST_WITHOUT_CAP', displayName: 'Lowest cost'
    LOWEST_COST_WITHOUT_CAP = Cat(_bid_strategy_sdk.lowest_cost_without_cap)
    LOWEST_COST_WITH_BID_CAP = Cat(_bid_strategy_sdk.lowest_cost_with_bid_cap)
    TARGET_COST = Cat(_bid_strategy_sdk.target_cost)
