...
class AdWordsAPIDataSyncHandler:
    ...
    def handle(cls, request):
        ...
        mongo_repository = GoogleBusinessOwnerMongoRepository(
            client=mongo_adapter.client,
        ...
        if active_google_accounts:
            client_customer_ids = [
                google_account["client_customer_id"]["google_id"] for google_account in active_google_accounts
            ]
            google_id_to_last_update_time = {
                google_account["client_customer_id"]["google_id"]: google_account["last_update_time"]
                for google_account in active_google_accounts
            }
            removed_customer_ids = list(set(client_customer_ids) - set(account_ids))
            mongo_repository.change_status_many(
                ids=removed_customer_ids,
                new_status=GoogleAccountStatus.REMOVED.value,
                id_key="client_customer_id.google_id",
            )

        mongo_structures_repository = GoogleTuringStructuresMongoRepository(
            client=mongo_adapter.client, database_name=config.mongo.google_structures_database_name
        )
        mongo_structures_repository.update_removed_structures(removed_customer_ids)

        for i, account_id in enumerate(account_ids):
            structures_synchronizer = StructuresSynchronizer(
            ...
            insights_synchronizer = InsightsSynchronizer(
                business_owner_id=bo_google_id,
                account_id=account_id,
                adwords_client=client,
                mongo_config=config.mongo,
            ...
            if last_update_time:
                query_filter = {"client_customer_id.google_id": {MongoOperator.EQUALS.value: account_id}}
                query = {MongoOperator.SET.value: google_account_doc}
