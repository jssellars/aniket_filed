...
from GoogleTuring.Infrastructure.Domain.Enums.Breakdown import (
    BREAKDOWN_TO_FIELD,
...
def create_join_dataframe(
    df_to_join, join_fields, breakdown, action_breakdown, level, report_downloader, report_name, start_date, end_date
):
    for join_field in join_fields:
        ...
        report_query = (
            ...
            .Where("CampaignStatus")
            .In("ENABLED", "PAUSED")
            .Where(compare_field.field_name)
            .EqualTo(equal_to)
            .During(start_date=start_date, end_date=end_date)
            .Build()
        )
...
class InsightsSynchronizer(BaseSynchronizer):
    def __init__(
        ...
        self.__mongo_repository = GoogleTuringInsightsMongoRepository(
            client=mongo_adapter.client,
            database_name=self._mongo_config.google_insights_database_name,
                ...
                for level in insights_definition.levels:
                    for breakdown in insights_definition.breakdowns:
                        for action_breakdown in insights_definition.action_breakdowns:
                            ...
                            try:
                                df = create_dataframe(
                                    filtered_fields, report_downloader, report_name, start_date, end_date
                                )
                                if join_fields:
                                    df = create_join_dataframe(
                                        df_to_join=df,
