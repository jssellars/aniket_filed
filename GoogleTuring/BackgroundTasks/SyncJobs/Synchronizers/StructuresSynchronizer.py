...
class StructuresSynchronizer(BaseSynchronizer):
    def __init__(self, business_owner_id, account_id, adwords_client, mongo_config, mongo_conn_handler):
        ...
        self.__mongo_repository = GoogleTuringStructuresMongoRepository(client=mongo_conn_handler.client, database_name=self._mongo_config['google_structures_database_name'])
    ...
    def synchronize(self):
        level_to_dependencies = {
            ...
            StructureType.AD_GROUP: (
                (self._adwords_client.get_ad_group_service(), self._adwords_client.get_ad_group_criterion_service()), (AD_GROUP_STRUCTURE_FIELDS, AD_GROUP_CRITERIA_FIELDS)),
            StructureType.AD: (self._adwords_client.get_ad_group_ad_service(), AD_STRUCTURE_FIELDS),
            StructureType.AD_GROUP_KEYWORDS: (self._adwords_client.get_ad_group_criterion_service(), AD_GROUP_KEYWORDS_STRUCTURE_FIELDS)
        }
        ...
        for level in LEVELS:
            print('[Structure BT] Business Owner: {} Level: {} account_id: {}'.format(self._business_owner_id, level, self._account_id))
            self.__mongo_repository.collection = level.value
            structure_mapping = structures_mapping_factory.get_structure_mapping(level=level, business_owner_id=self._business_owner_id,
