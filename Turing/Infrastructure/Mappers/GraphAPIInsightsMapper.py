...
from Turing.Infrastructure.Mappers.ObjectiveToResultsMapper import ObjectiveToResultsMapper
...
class GraphAPIInsightsMapper:
    ...
    action_value_mapper_factory = {
        FacebookFieldsMetadata.results.name: ObjectiveToResultsMapper,
        FacebookFieldsMetadata.cost_per_result.name: ObjectiveToResultsMapper
    }

    ids_map = [FacebookFieldsMetadata.ad_account_id.name,
               FacebookFieldsMetadata.campaign_id.name,
               FacebookFieldsMetadata.adset_id.name,
               FacebookFieldsMetadata.ad_id.name]

    @classmethod
    def mappers_factory(cls):
        mappers_factory = {
            FacebookFieldType.SIMPLE: cls.map_simple,
            FacebookFieldType.NESTED: cls.map_nested,
            FacebookFieldType.TOTAL: cls.map_total,
            FacebookFieldType.COST: cls.map_cost,
            FacebookFieldType.BREAKDOWN: cls.map_breakdown,
            FacebookFieldType.ACTION_BREAKDOWN: cls.map_simple,
            FacebookFieldType.TIME_BREAKDOWN: cls.map_time_breakdown,
            FacebookFieldType.STRUCTURE: cls.map_simple
        }
    ...
    def map_response(cls, requested_fields, response):
        ...
        action_breakdowns = [field for field in requested_fields if field.field_type == FacebookFieldType.ACTION_BREAKDOWN and field.name != FacebookFieldsMetadata.action_type.name]

        if action_breakdowns:
            nested_fields = [field for field in requested_fields if field.field_type == FacebookFieldType.NESTED]
    ...
    def __is_convertible(cls, field):
        return field.field_name not in cls.ids_map and field.field_type not in [FacebookFieldType.BREAKDOWN, FacebookFieldType.ACTION_BREAKDOWN, FacebookFieldType.TIME_BREAKDOWN]

    @classmethod
    def map_simple(cls, field, response):

        if field.field_type != FacebookFieldType.STRUCTURE:
            try:
    ...
    def map_cost(cls, field, response):
        if field.name == FacebookFieldsMetadata.cost_per_1000_people_reached.name:
    ...
    def map_cost_per_1000_people_reached(cls, field, response):
        try:
            return response[FacebookFieldsMetadata.social_spend.name] / response[FacebookFieldsMetadata.reach.name] * 1000
        except Exception as e:
            print(e)
