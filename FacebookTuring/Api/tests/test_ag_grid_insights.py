...
class TestAgGridInsights:
    ...
    post_data = {
        ...
        "sortModel": [],
        "agColumns": "selected,status,adset_id,adset_name,effective_status,budget,amount_spent,impressions,cpm,frequency,unique_clicks_all,cost_per_unique_click_all,unique_ctr_all,results,cost_per_result,conversion_rate_ranking,landing_page_views_total,landing_page_views_unique,purchases_value,purchase_roas,stop_time",
    ...
    def response(self, client, config):
        url = f"/api/v1/ag-grid-insights/adset"
...
class TestAgGridInsightsDataParams(TestAgGridInsightsSummaryParam):
    ...
    def test_budget_is_non_empty_string(self, data):
        for response in data:
            assert AgGridInsightField.BUDGET.value in response
    ...
    def test_cost_per_result_is_positive_float(self, data):
        for response in data:
            assert AgGridInsightField.COST_PER_RESULT.value in response
    ...
    def test_results_is_positive_float(self, data):
        for response in data:
            assert AgGridInsightField.RESULTS.value in response
    ...
    def test_account_id_is_non_empty_numeric_string(self, data):
        for response in data:
            assert AgGridInsightField.ADSET_ID.value in response
    ...
    def test_account_name_is_non_empty_string(self, data):
        for response in data:
            assert AgGridInsightField.ACCOUNT_NAME.value in response
    ...
    def test_campaign_id_is_non_empty_numeric_string(self, data):
        for response in data:
            assert AgGridInsightField.CAMPAIGN_ID.value in response
    ...
    def test_campaign_name_is_non_empty_string(self, data):
        for response in data:
            assert AgGridInsightField.CAMPAIGN_NAME.value in response
    ...
    def test_daily_budget_is_positive_float(self, data):
        for response in data:
            assert AgGridInsightField.DAILY_BUDGET.value in response
    ...
    def test_id_is_non_empty_string(self, data):
        for response in data:
            assert AgGridInsightField.ID.value in response
            _id = response.get(AgGridInsightField.ID.value)
            if _id is not None:
                assert isinstance(_id, str)
                assert len(_id) > 0

    def test_lifetime_budget_is_positive_float(self, data):
        for response in data:
            assert AgGridInsightField.LIFETIME_BUDGET.value in response
            ...
            if lifetime_budget is not None and lifetime_budget is not 0:
                assert isinstance(lifetime_budget, float)
                assert lifetime_budget >= 0

    def test_name_is_non_empty_string(self, data):
        for response in data:
            assert AgGridInsightField.NAME.value in response
            name = response.get(AgGridInsightField.NAME.value)
            if name is not None:
