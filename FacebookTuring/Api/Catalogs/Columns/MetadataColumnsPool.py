...
from FacebookTuring.Api.Catalogs.Columns.AggregationType import AggregationType
...
class MetadataColumnsPool():
    ...
    ad_account_id = ColumnMetadata(FieldsMetadata.ad_account_id.name, AggregationType.null, ColumnType.text)

    account_name = ColumnMetadata(FieldsMetadata.account_name.name, AggregationType.null, ColumnType.text)

    ad_id = ColumnMetadata(FieldsMetadata.ad_id.name, AggregationType.null, ColumnType.text)

    ad_name = ColumnMetadata(FieldsMetadata.ad_name.name, AggregationType.null, ColumnType.text)

    adset_id = ColumnMetadata(FieldsMetadata.adset_id.name, AggregationType.null, ColumnType.text)

    adset_name = ColumnMetadata(FieldsMetadata.adset_name.name, AggregationType.null, ColumnType.text)

    buying_type = ColumnMetadata(FieldsMetadata.buying_type.name, AggregationType.null, ColumnType.text)

    campaign_id = ColumnMetadata(FieldsMetadata.campaign_id.name, AggregationType.null, ColumnType.text)

    campaign_name = ColumnMetadata(FieldsMetadata.campaign_name.name, AggregationType.null, ColumnType.text)

    effective_status = ColumnMetadata(FieldsMetadata.effective_status.name, AggregationType.null, ColumnType.text)

    tags = ColumnMetadata(FieldsMetadata.tags.name, AggregationType.null, ColumnType.text)

    objective = ColumnMetadata(FieldsMetadata.objective.name, AggregationType.null, ColumnType.text)

    created_at = ColumnMetadata(FieldsMetadata.created_at.name, AggregationType.null, ColumnType.date)

    last_significant_edit = ColumnMetadata(FieldsMetadata.last_significant_edit.name, AggregationType.null, ColumnType.date)

    start_date = ColumnMetadata(FieldsMetadata.start_date.name, AggregationType.null, ColumnType.date)

    end_date = ColumnMetadata(FieldsMetadata.end_date.name, AggregationType.null, ColumnType.date)

    bid_strategy = ColumnMetadata(FieldsMetadata.bid_strategy.name, AggregationType.null, ColumnType.text)

    amount_spent_percentage = ColumnMetadata(FieldsMetadata.amount_spent_percentage.name, AggregationType.avg, ColumnType.number)

    bid_cap = ColumnMetadata(FieldsMetadata.bid_cap.name, AggregationType.avg, ColumnType.number)

    budget = ColumnMetadata(FieldsMetadata.budget.name, AggregationType.sum, ColumnType.number)

    budget_remaining = ColumnMetadata(FieldsMetadata.budget_remaining.name, AggregationType.sum, ColumnType.number)

    # Targeting

    location = ColumnMetadata(FieldsMetadata.location.name, AggregationType.null, ColumnType.text)

    age = ColumnMetadata(FieldsMetadata.age.name, AggregationType.null, ColumnType.text)

    gender = ColumnMetadata(FieldsMetadata.gender.name, AggregationType.null, ColumnType.text)

    included_custom_audiences = ColumnMetadata(FieldsMetadata.included_custom_audiences.name, AggregationType.null, ColumnType.text)

    excluded_custom_audiences = ColumnMetadata(FieldsMetadata.excluded_custom_audiences.name, AggregationType.null, ColumnType.text)

    # Ad creative

    page_name = ColumnMetadata(FieldsMetadata.page_name.name, AggregationType.null, ColumnType.text)

    headline = ColumnMetadata(FieldsMetadata.headline.name, AggregationType.null, ColumnType.text)

    body = ColumnMetadata(FieldsMetadata.body.name, AggregationType.null, ColumnType.text)

    link = ColumnMetadata(FieldsMetadata.link.name, AggregationType.null, ColumnType.text)

    destination = ColumnMetadata(FieldsMetadata.destination.name, AggregationType.null, ColumnType.text)

    # Tracking
    url_parameters = ColumnMetadata(FieldsMetadata.url_parameters.name, AggregationType.null, ColumnType.text)

    pixel = ColumnMetadata(FieldsMetadata.pixel.name, AggregationType.null, ColumnType.text)

    app_event = ColumnMetadata(FieldsMetadata.app_event.name, AggregationType.null, ColumnType.text)

    offline_event = ColumnMetadata(FieldsMetadata.offline_event.name, AggregationType.null, ColumnType.text)
    ...
    canvas_avg_view_percent = ColumnMetadata(FieldsMetadata.instant_experience_view_percentage.name, AggregationType.avg, ColumnType.number)

    canvas_avg_view_time = ColumnMetadata(FieldsMetadata.instant_experience_view_time.name, AggregationType.avg, ColumnType.number)

    clicks = ColumnMetadata(FieldsMetadata.all_clicks.name, AggregationType.sum, ColumnType.number)

    conversion_rate_ranking = ColumnMetadata(FieldsMetadata.conversion_rate_ranking.name, AggregationType.null, ColumnType.text)

    cost_per_estimated_ad_recallers = ColumnMetadata(FieldsMetadata.cost_per_estimated_ad_recall_lift.name, AggregationType.avg, ColumnType.number)
    ...
    cost_per_unique_click = ColumnMetadata(FieldsMetadata.cost_per_unique_click_all.name, AggregationType.avg, ColumnType.number)

    # costPerUniqueInlineLinkClick = ColumnMetadata("cost_per_unique_inline_link_click", AggregationType.avg, ColumnType.number)

    cpc = ColumnMetadata(FieldsMetadata.all_cpc.name, AggregationType.avg, ColumnType.number)

    cpm = ColumnMetadata(FieldsMetadata.cpm.name, AggregationType.avg, ColumnType.number)

    cpp = ColumnMetadata(FieldsMetadata.all_cpp.name, AggregationType.avg, ColumnType.number)

    ctr = ColumnMetadata(FieldsMetadata.all_ctr.name, AggregationType.avg, ColumnType.number)

    date_start = ColumnMetadata(FieldsMetadata.date_start.name, AggregationType.null, ColumnType.date)

    date_stop = ColumnMetadata(FieldsMetadata.date_stop.name, AggregationType.null, ColumnType.date)

    engagement_rate_ranking = ColumnMetadata(FieldsMetadata.engagement_rate_ranking.name, AggregationType.null, ColumnType.text)

    estimated_ad_recall_rate = ColumnMetadata(FieldsMetadata.estimated_ad_recall_rate.name, AggregationType.avg, ColumnType.number)

    estimated_ad_recallers = ColumnMetadata(FieldsMetadata.estimated_ad_recall_lift.name, AggregationType.sum, ColumnType.number)

    frequency = ColumnMetadata(FieldsMetadata.frequency.name, AggregationType.avg, ColumnType.number)

    full_view_impressions = ColumnMetadata(FieldsMetadata.full_view_impressions.name, AggregationType.sum, ColumnType.number)

    full_view_reach = ColumnMetadata(FieldsMetadata.full_view_reach.name, AggregationType.sum, ColumnType.number)

    impressions = ColumnMetadata(FieldsMetadata.impressions.name, AggregationType.sum, ColumnType.number)
    ...
    instant_experience_clicks_to_open = ColumnMetadata(FieldsMetadata.instant_experience_click_to_open.name, AggregationType.sum, ColumnType.number)

    instant_experience_clicks_to_start = ColumnMetadata(FieldsMetadata.instant_experience_click_to_start.name, AggregationType.sum, ColumnType.number)

    instant_experience_outbound_clicks = ColumnMetadata(FieldsMetadata.instant_experience_outbound_click.name, AggregationType.sum, ColumnType.number)

    # placePageName = ColumnMetadata("place_page_name", AggregationType.null, ColumnType.text)

    quality_ranking = ColumnMetadata(FieldsMetadata.quality_ranking.name, AggregationType.null, ColumnType.text)

    reach = ColumnMetadata(FieldsMetadata.reach.name, AggregationType.sum, ColumnType.number)

    social_spend = ColumnMetadata(FieldsMetadata.social_spend.name, AggregationType.sum, ColumnType.number)

    spend = ColumnMetadata(FieldsMetadata.amount_spent.name, AggregationType.sum, ColumnType.number)

    unique_clicks = ColumnMetadata(FieldsMetadata.unique_click.name, AggregationType.sum, ColumnType.number)

    unique_ctr = ColumnMetadata(FieldsMetadata.unique_ctr.name, AggregationType.avg, ColumnType.number)
    ...
    unique_link_clicks_ctr = ColumnMetadata(FieldsMetadata.unique_link_click_ctr.name, AggregationType.sum, ColumnType.number)

    # Page post

    page_engagement = ColumnMetadata(FieldsMetadata.page_engagement.name, AggregationType.sum, ColumnType.number)

    like = ColumnMetadata(FieldsMetadata.page_like.name, AggregationType.sum, ColumnType.number)

    comment = ColumnMetadata(FieldsMetadata.post_comment.name, AggregationType.sum, ColumnType.number)

    post_engagement = ColumnMetadata(FieldsMetadata.post_engagement.name, AggregationType.sum, ColumnType.number)

    post_reaction = ColumnMetadata(FieldsMetadata.post_reaction.name, AggregationType.sum, ColumnType.number)

    # onsiteConversion = ColumnMetadata("actions_onsite_conversion_post_save", AggregationType.sum, ColumnType.number)

    post_share = ColumnMetadata(FieldsMetadata.post_share.name, AggregationType.sum, ColumnType.number)

    photo_view = ColumnMetadata(FieldsMetadata.photo_view.name, AggregationType.sum, ColumnType.number)

    event_responses = ColumnMetadata(FieldsMetadata.event_responses.name, AggregationType.sum, ColumnType.number)

    # effectShare = ColumnMetadata("effect_share", AggregationType.sum, ColumnType.number)

    cost_per_page_engagement = ColumnMetadata(FieldsMetadata.cost_per_page_engagement.name, AggregationType.avg, ColumnType.number)

    cost_per_like = ColumnMetadata(FieldsMetadata.cost_per_page_like.name, AggregationType.avg, ColumnType.number)

    cost_per_post_engagement = ColumnMetadata(FieldsMetadata.cost_per_post_engagement.name, AggregationType.avg, ColumnType.number)

    cost_per_event_response = ColumnMetadata(FieldsMetadata.cost_per_event_response.name, AggregationType.avg, ColumnType.number)

    # Messaging

    messaging_block = ColumnMetadata(FieldsMetadata.blocked_messaging_connections.name, AggregationType.sum, ColumnType.number)

    messaging_conversation_started_7d = ColumnMetadata(FieldsMetadata.messaging_conversation_started_7d.name, AggregationType.sum, ColumnType.number)

    messaging_first_reply = ColumnMetadata(FieldsMetadata.new_messaging_connections.name, AggregationType.sum, ColumnType.number)
    ...
    cost_per_messaging_first_reply = ColumnMetadata(FieldsMetadata.cost_per_new_messaging_connection.name, AggregationType.sum, ColumnType.number)
    ...
    total_video_p100_watched_actions = ColumnMetadata(FieldsMetadata.total_video_100p_watched_actions.name, AggregationType.sum, ColumnType.number)

    total_video_p25_watched_actions = ColumnMetadata(FieldsMetadata.total_video_25p_watched_actions.name, AggregationType.sum, ColumnType.number)

    total_video_p50_watched_actions = ColumnMetadata(FieldsMetadata.total_video_50p_watched_actions.name, AggregationType.sum, ColumnType.number)

    total_video_p75_watched_actions = ColumnMetadata(FieldsMetadata.total_video_75p_watched_actions.name, AggregationType.sum, ColumnType.number)

    total_video_p95_watched_actions = ColumnMetadata(FieldsMetadata.total_video_95p_watched_actions.name, AggregationType.sum, ColumnType.number)

    video_play_actions = ColumnMetadata(FieldsMetadata.video_play.name, AggregationType.sum, ColumnType.number)

    video_thruplay_watched_actions = ColumnMetadata(FieldsMetadata.thruplay.name, AggregationType.sum, ColumnType.number)

    link_click_website_ctr = ColumnMetadata(FieldsMetadata.link_click_ctr.name, AggregationType.sum, ColumnType.number)

    website_purchase_roas = ColumnMetadata(FieldsMetadata.website_purchase_roas.name, AggregationType.sum, ColumnType.number)

    purchase_roas = ColumnMetadata(FieldsMetadata.purchase_roas.name, AggregationType.sum, ColumnType.number)

    #  Clicks

    link_click = ColumnMetadata(FieldsMetadata.link_click.name, AggregationType.sum, ColumnType.number)

    unique_link_click = ColumnMetadata(FieldsMetadata.unique_link_click.name, AggregationType.sum, ColumnType.number)

    outbound_click = ColumnMetadata(FieldsMetadata.outbound_click.name, AggregationType.sum, ColumnType.number)

    unique_outbound_click = ColumnMetadata(FieldsMetadata.unique_outbound_click.name, AggregationType.sum, ColumnType.number)

    outbound_link_click_ctr = ColumnMetadata(FieldsMetadata.outbound_link_click_ctr.name, AggregationType.avg, ColumnType.number)

    unique_outbound_link_click_ctr = ColumnMetadata(FieldsMetadata.unique_outbound_link_click_ctr.name, AggregationType.avg, ColumnType.number)

    cost_per_outbound_click = ColumnMetadata(FieldsMetadata.cost_per_outbound_link_click.name, AggregationType.avg, ColumnType.number)

    cost_per_unique_outbound_click = ColumnMetadata(FieldsMetadata.click_per_unique_outbound_link_click.name, AggregationType.avg, ColumnType.number)
    ...
    #  ====== Calculated metrics ====== #
    cost_per_1000_people_reached = ColumnMetadata(FieldsMetadata.cost_per_1000_people_reached.name, AggregationType.avg, ColumnType.number)

    cost_per_video_thruplay_watched_actions = ColumnMetadata(FieldsMetadata.cost_per_thruplay.name, AggregationType.avg, ColumnType.number)

    results = ColumnMetadata(FieldsMetadata.results.name, AggregationType.sum, ColumnType.number)

    cost_per_result = ColumnMetadata(FieldsMetadata.cost_per_result.name, AggregationType.avg, ColumnType.number)

    conversions = ColumnMetadata(FieldsMetadata.conversions.name, AggregationType.sum, ColumnType.number)

    cost_per_conversion = ColumnMetadata(FieldsMetadata.cost_per_conversion.name, AggregationType.sum, ColumnType.number)
