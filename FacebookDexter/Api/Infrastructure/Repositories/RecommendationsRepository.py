...
class RecommendationsRepository(object):
    ...
    def set_recommendation_statuses_by_structure_id(self, structure_id, status):
        now = datetime.now()
        return self.collection.update_many({"structure_id": structure_id, "status": {"$ne": RecommendationStatus.APPLIED.value}},
                                           {'$set': {"status": status, "application_date": now, 'applied_by': 'Dexter'}})
    ...
    def get_action_history(self, structureId: str):
        cursor = self.collection.find(
            {"structure_id": structureId, "status": {"$in": [RecommendationStatus.APPLIED.value, RecommendationStatus.DISMISSED.value]}},
