...
class RecommendationsRepository(object):
    ...
    def __init__(self, config: MongoConfig):
        ...
        server.start()
        self.client = pymongo.MongoClient(host='127.0.0.1', port=server.local_bind_port, username=config.mongoUser,
    ...
    def get_recommendations_by_ad_account_and_level(self, adAccountId, level, channel):
        ...
        recommendationsAsDictList = [Recommendation(retrievedRecommendation).__dict__ for retrievedRecommendation in
                                     list(cursor)]
    ...
    def set_recommendation_status(self, id: str, status: str):
        ...
        self.collection.update_one({"_id": recommendation_objectId},
                                   {'$set': {"status": status, "applicationDate": now, 'appliedBy': 'Dexter'}})
    ...
    def get_action_history(self, structureId: str):
        cursor = self.collection.find({"structureId": structureId, "status": {
            "$in": [RecommendationStatus.APPLIED.value, RecommendationStatus.DISMISSED.value]}},
                                      {'applicationDetails': False}).sort("applicationDate", pymongo.DESCENDING)
        actionHistorysAsDictList = [Recommendation(retrievedRecommendation).__dict__ for retrievedRecommendation in
                                    list(cursor)]
